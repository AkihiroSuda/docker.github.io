...
> Rootless mode is planned to graduate experimental in Docker Engine 20.0X.
...
- Only `vfs` graphdriver is supported. However, on Ubuntu and Debian 10,
  `overlay2` and `overlay` are also supported.
   Docker 20.0X will additionally support `fuse-overlayfs` driver to provide
   overlayfs functionality for any distribution with kernel 4.18 or later.

- Following features are not supported:
  - Cgroups (including `docker top`, which depends on the cgroups.
    Will be supported in Docker 20.0X with containerd 1.4.)
  - AppArmor
  - Checkpoint
  - Overlay network
  - Exposing SCTP ports
...
- Host network (`docker run --net=host`) is namespaced inside RootlessKit and works in mostly unexpected way.
...
`--cpus`, `--memory`, --pids-limit`. (Will be supported in Docker 20.0X with containerd 1.4).
...
`--experimental` will no longer be needed since Docker 20.0X.
...
cgroup v2 will be supported in Docker 20.0X with containerd 1.4.
